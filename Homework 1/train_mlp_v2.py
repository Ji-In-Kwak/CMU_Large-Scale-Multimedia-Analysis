...
if __name__ == '__main__':
  ...
  clf = MLPClassifier(hidden_layer_sizes=(1024, 512, 256, 128),activation="relu",solver="adam",alpha=1e-3)
  ...
  #2. Test validation sets
  fread = open(args.val_list_videos, "r")
  feat_list = []
  # labels are [0-9]
  label_list = []
  # load video names and events in dict
  df_videos_label = {}
  for line in open(args.val_list_videos).readlines()[1:]:
    video_id, category = line.strip().split(",")
    df_videos_label[video_id] = category
  ...
  ## Validation score
  pred_classes = clf.predict(feat_list)
  accuracy = accuracy_score(label_list, pred_classes)
  print(accuracy)