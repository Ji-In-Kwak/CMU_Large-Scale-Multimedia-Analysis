...
def load_audio(audio_path, sample_rate=22000, mono=True):
    ...
    if np.max(np.abs(sound_sample), axis=0) != 0:
        sound_sample /= np.max(np.abs(sound_sample),axis=0)
